...
void ZipCodeTree::print_self(const vector<Seed>* seeds, const VectorView<Minimizer>* minimizers) const {
    for (const tree_item_t item : zip_code_tree) {
        if (item.get_type() == SEED) {
            ...
            cerr << " " << item.get_value() << "-" << item.get_value() + item.get_distance_difference() << " ";
    ...
    if (depth != 0) {
        ...
        if (depth != child_depth) {
            ...
            if (chain_is_reversed) {
                ...
                max_prefix_sum = SnarlDistanceIndex::minus(SnarlDistanceIndex::minus(
                                    forest_state.distance_index->maximum_length(forest_state.distance_index->get_parent(child_handle)),
                                    max_prefix_sum),
                                    forest_state.distance_index->maximum_length(child_handle));
            }
            cerr << "FOR CHILD " << forest_state.distance_index->net_handle_as_string(child_handle) << " add max distance " << max_prefix_sum << " " << min_prefix_sum << endl;
            cerr << "FROM VALUES " << forest_state.distance_index->maximum_length(forest_state.distance_index->get_parent(child_handle)) << ", "
                 << " " <<  forest_state.distance_index->get_max_prefix_sum_value(child_handle) << " and " << forest_state.distance_index->maximum_length(forest_state.distance_index->get_parent(child_handle)) << endl;
            forest_state.sibling_indices_at_depth[depth-1].back().max_distances.first 
                = forest_state.sibling_indices_at_depth[depth-1].back().distances.first + max_prefix_sum-min_prefix_sum; 
        } else {
            cerr << "ACTUALLY THIS IS A TRIVIAL CHAIN" << endl;
            forest_state.sibling_indices_at_depth[depth-1].back().max_distances.first 
                = forest_state.sibling_indices_at_depth[depth-1].back().distances.first; 
        }
...
void ZipCodeForest::close_chain(forest_growing_state_t& forest_state, 
...
#endif
    cerr << "OLD MAX DISTANCE " << forest_state.sibling_indices_at_depth[depth].back().max_distances.first << endl;
    if (trees[forest_state.active_tree_index].zip_code_tree.back().get_type() == ZipCodeTree::CHAIN_START) {
        ...
        cerr << "For chain " << forest_state.distance_index->net_handle_as_string(chain_handle) << " adding max distance to end " << max_distance_to_chain_end << endl;
        ...
        if (distance_to_chain_end > forest_state.distance_limit && forest_state.open_chains.back().second) {
            ...
            if (trees[forest_state.active_tree_index].zip_code_tree.at(forest_state.open_chains.back().first).get_type() 
                ...
                size_t child_depth = last_seed.zipcode_decoder->max_depth();
                net_handle_t child_handle = forest_state.distance_index->get_node_net_handle(id(last_seed.pos));
                while (child_depth > depth+1) {
                    child_handle = forest_state.distance_index->get_parent(child_handle);
                    if (forest_state.distance_index->is_trivial_chain(child_handle)) {
                        child_handle = forest_state.distance_index->get_parent(child_handle);
                ...
                size_t max_last_length = forest_state.distance_index->maximum_length(child_handle);

                max_distance_to_chain_end = SnarlDistanceIndex::sum(max_distance_to_chain_end,
                                            SnarlDistanceIndex::sum(last_edge,
                                            max_last_length));
            }
        }
        if (add_distances) {
            ...
            cerr << "ADDING DISTANCES TO END " << distance_to_chain_end << " AND " << max_distance_to_chain_end << endl;
    ...
    cerr << "NEW MAX DISTANCE " << forest_state.sibling_indices_at_depth[depth].back().max_distances.first << endl;
}
...
void ZipCodeForest::add_child_to_chain(forest_growing_state_t& forest_state, 
    ...
    if (current_type == ZipCode::ROOT_NODE || current_type == ZipCode::NODE || is_trivial_chain) {
        ...
        if (current_type == ZipCode::NODE) {
            ...
            cerr << "For child " << forest_state.distance_index->net_handle_as_string(node_handle) << " max offset " << max_current_offset << endl;
            cerr << "At chain depth " << chain_depth << endl;
        } 
    } else {
        ...
        cerr << "At child " << forest_state.distance_index->net_handle_as_string(child_handle) << endl;
    ...
    assert(max_previous_offset >= previous_offset);
    assert(max_current_offset >= current_offset);
#endif
        ...
        if (chain_depth == 0 && distance_between > forest_state.distance_limit) {
            ...
            if (forest_state.open_chains.back().second) {
                ...
                if (trees[forest_state.active_tree_index].zip_code_tree.at(forest_state.open_chains.back().first).get_type() 
                    ...
                    cerr << "UPDATE OFFSETS: " << current_offset << " " << max_current_offset << endl;
...
#ifdef DEBUG_ZIP_CODE_TREE
    cerr << "Add sibling with type " << current_type << " with max offset " << max_current_offset << endl;
#endif
...
void ZipCodeForest::close_snarl(forest_growing_state_t& forest_state, 
    ...
    if (depth == 0) {
        ...
        if (trees[forest_state.active_tree_index].zip_code_tree.back().get_type() == ZipCodeTree::EDGE) {
            ...
            size_t max_previous_edge = SnarlDistanceIndex::sum(previous_edge,
                                       trees[forest_state.active_tree_index].zip_code_tree.back().get_distance_difference());
            trees[forest_state.active_tree_index].zip_code_tree.pop_back();
            ...
            size_t max_snarl_prefix_sum = forest_state.sibling_indices_at_depth[depth-1].back().distances.first;
            forest_state.sibling_indices_at_depth[depth-1].pop_back();
            ...
            size_t child_depth = last_seed.zipcode_decoder->max_depth();
            net_handle_t snarl_handle = forest_state.distance_index->get_node_net_handle(id(last_seed.pos));
            while (child_depth > depth) {
            ...
            forest_state.sibling_indices_at_depth[depth-1].push_back({
                ...
                std::make_pair(SnarlDistanceIndex::minus(max_snarl_prefix_sum, max_previous_edge), 0)});
...
void ZipCodeForest::add_snarl_distances(forest_growing_state_t& forest_state, const size_t& depth, 
    ...
    size_t max_distance_to_chain_start = to_snarl_end ? 0 : forest_state.sibling_indices_at_depth[depth].back().max_distances.first;
    ...
    for ( size_t sibling_i = 0 ; sibling_i < sibling_count ; sibling_i++) {
        ...
        if (sibling.type == ZipCodeTree::SNARL_START && !is_cyclic_snarl) {
            ...
            if (to_snarl_end) {
                ...
                size_t max_snarl_distance = SnarlDistanceIndex::sum (max_distance_to_chain_start,
                                                    snarl_is_reversed
                                                        ? seed.zipcode_decoder->get_distance_to_snarl_end(depth+1)
                                                        : seed.zipcode_decoder->get_distance_to_snarl_start(depth+1));
                cerr << "ADDING MAX DISTANCE: " << max_snarl_distance << " With normal distance " << snarl_distance << endl;
                cerr << "From: " << max_distance_to_chain_start << endl;
                assert(max_snarl_distance >= snarl_distance);
                max_distance = max_snarl_distance - snarl_distance;
            }
            ...
            if (seed.zipcode_decoder->get_code_type(depth) == ZipCode::REGULAR_SNARL) {
                ...
                max_distance = to_snarl_end ? sibling.max_distances.second : std::numeric_limits<size_t>::max();
                cerr <<"Max distance from sibling " << sibling.max_distances.second << endl;
            } else {
                ...
                if (to_snarl_end && !is_cyclic_snarl) {
                    ...
                    max_distance = SnarlDistanceIndex::sum(sibling.max_distances.second,
                                          snarl_is_reversed ? sibling_seed.zipcode_decoder->get_distance_to_snarl_start(depth+1)
                                                            : sibling_seed.zipcode_decoder->get_distance_to_snarl_end(depth+1));
                    cerr << "Max distance with sibling's max distance " << sibling.max_distances.second << endl;
                } else {
                    ...
                    size_t max_distance_to_end_of_last_child = sibling.type == ZipCodeTree::SNARL_START ? 0
                                                     : sibling.max_distances.second;
                    ...
                    size_t distance_in_snarl = forest_state.distance_index->distance_in_snarl(snarl_handle, rank1, right_side1, rank2, right_side2);
                    distance = SnarlDistanceIndex::sum(SnarlDistanceIndex::sum(distance_in_snarl,
                                                                               distance_to_chain_start),
                                                                               distance_to_end_of_last_child);
                    cerr << "ADD MAX DISTANCE TO START " << max_distance_to_chain_start << endl;
            ...
            trees[forest_state.active_tree_index].zip_code_tree.at(last_child_index - 1 - sibling_i) 
                    = {ZipCodeTree::EDGE, distance, false, max_distance - distance};
        }
