...
void ZipCodeForest::close_chain(forest_growing_state_t& forest_state, 
    ...
    if (trees[forest_state.active_tree_index].zip_code_tree.back().get_type() == ZipCodeTree::CHAIN_START) {
        ...
        size_t max_distance_to_chain_end = SnarlDistanceIndex::minus(forest_state.distance_index->maximum_length(chain_handle),
                                      forest_state.sibling_indices_at_depth[depth].back().max_distances.first);
...
void ZipCodeForest::add_child_to_chain(forest_growing_state_t& forest_state, 
    ...
    size_t max_previous_offset = previous_offset == 0 ? 0 : forest_state.sibling_indices_at_depth[chain_depth][0].max_distances.first;
#ifdef DEBUG_ZIP_CODE_TREE
...
void ZipCodeForest::close_snarl(forest_growing_state_t& forest_state, 
    ...
    if (depth == 0) {
        ...
        if (trees[forest_state.active_tree_index].zip_code_tree.back().get_type() == ZipCodeTree::EDGE) {
            ...
            size_t max_snarl_prefix_sum = forest_state.sibling_indices_at_depth[depth-1].back().max_distances.first;
            forest_state.sibling_indices_at_depth[depth-1].pop_back();
        ...
        for (reverse_iterator tree_itr_left (start_itr_left, zip_code_tree.rend()) ;
            ...
            if (in_non_dag_snarl) {
                ...
                if (start_pos == next_pos) {
                    ...
                    if (id(next_seed.pos) != id(start_seed.pos)) {
                        assert((max_tree_distance == max_index_distance || max_tree_distance+1 == max_index_distance || max_tree_distance == max_index_distance+1));
                    }
                }
            }
