    ...
    private:
        ...
        struct NetgraphNode {
            ...
            size_t rank_in_parent(MinimumDistanceIndex& dist_index, id_t id) const {
                ...
                size_t rank = node_type == NODE ? dist_index.get_primary_snarl_rank(id) :
                                                  dist_index.get_secondary_snarl_rank(id);
                if ( (node_type == SNARL && 
                      dist_index.snarl_indexes[dist_index.get_primary_snarl_assignment(id)].rev_in_parent) ||
                     (node_type == CHAIN && 
